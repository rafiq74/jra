...
$urlparams = $_GET;
$PAGE->set_url('/local/jra/application/applicant/admitted.php', $urlparams);
...
jra_ui_show_flash_message('jra_admission_updated');
...
$aggregate = '';
$per_page = jra_ui_filter_value('per_page', jra_global_var('PER_PAGE'), 'admittedlist', $per_page_list, false);
...
//	if($status == 5)
	{
		$action_item[] = array(
			'title' => jra_get_string(['not', 'admit', 'all', 'in_the_list']), // - for divider
			'url' => "javascript:update_confirm_status_all(8, '" . $PAGE->url->out(false) . "')",
			'target' => '', //_blank
			'icon' => 'check-circle',
		);
		$action_item[] = array(
		...
		$action_item[] = array(
			...
			'url' => "javascript:update_confirm_status_all(20, '" . $PAGE->url->out(false) . "')",
			'target' => '', //_blank
			'icon' => 'send',
		);
	}
...
$excel_url = new moodle_url('admitted_excel.php');
$master_filter = $master_filter . html_writer::link($excel_url, jra_ui_icon('file-excel-o', 1, true), array('title' => get_string('download_excel_report', 'local_jra'), 'target' => '_blank'));
$master_filter = $master_filter . jra_ui_space(3);
$csv_url = new moodle_url('admitted_csv.php');
$master_filter = $master_filter . html_writer::link($csv_url, jra_ui_icon('file-code-o', 1, true), array('title' => 'Download CSV report', 'target' => '_blank'));
$master_filter = $master_filter . jra_ui_space(3);
$update_url = new moodle_url('update_id.php');
$master_filter = $master_filter . html_writer::link($update_url, jra_ui_icon('address-card', 1, true), array('title' => 'Update Trainee ID'));
$master_filter = $master_filter . '</span>';
$master_filter = $master_filter . jra_ui_hidden('status', '');
...
$conditionWhere = " institute = '" . jra_get_institute() . "' and semester = '" . $semester->semester . "' and deleted = 0 and status = 11 and admit_status = '1' $status_filter $city_filter";
...
$fields = array(
	...
	'placement_test_score' => array(
		'header'=>jra_get_string(['placement_test_score']), //for custom header
		'align' => 'center',
		'size' => '10%',
	),
	'admit_status_date' => array(
	...
	'idnumber' => array(
		'header'=>jra_get_string(['trainee', 'id']), //for custom header
		'align' => 'left',
		'size' => '10%',
	),
	'*' => array(), //action
...
//output the table
echo jra_ui_dump_table('si_applicant_admitted', $options, $fields, 'local_jra');
