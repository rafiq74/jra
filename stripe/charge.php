...
$data->instance_cost       = (float)$custom[1];
...
try {
    ...
    $charge = Stripe_Charge::create(array(
      ...
      "description" => 'Subscription for JRA Pro Package',
...
*/
    ...
    $destination = "$CFG->wwwroot/local/jra/index.php";

    $fullname = format_string('JRA Pro Package');

	//check if user is enrolled in the package
    redirect($destination, get_string('paymentthanks', '', $fullname));
	/*
    if (is_enrolled_in_package()) { // TODO: use real stripe check.
        redirect($destination, get_string('paymentthanks', '', $fullname));
