...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/program/program_status.php', $urlparams);
...
if(sis_is_system_admin() && isset($_POST['delete_id'])) //only allow site admin to delete
{
	...
	sis_query_delete_cascade('si_program_status', $_POST['delete_id'], $cascade);
}


echo $OUTPUT->header();
//content code starts here
sis_ui_page_title(get_string('program_status','local_sis'));
$currenttab = 'program_status'; //change this according to tab
include('tabs.php');
echo $OUTPUT->box_start('sis_tabbox');

$add_url = new moodle_url('/local/sis/setting/program/add_program_status.php', array('action' => '1'));
echo '<div class="pull-right rc-attendance-teacher-print">' . html_writer::link($add_url, sis_ui_icon('plus-circle', '1', true) . ' ' . get_string('add_program_status', 'local_sis'), array('title' => get_string('add_program_status', 'local_sis'))) . '</div>';
...
$options = array(
	...
	'default_search_field' => 'program_status', //default field choose for search
	'view_page' => '',
	'edit_page' => 'add_program_status.php',
...
//output the table
echo sis_ui_dump_table('si_program_status', $options, $fields, 'local_sis');
