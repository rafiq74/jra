...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/program/academic_career.php', $urlparams);
...
$PAGE->navbar->add(get_string('academic_career', 'local_sis'), new moodle_url('academic_career.php'));
...
if(sis_is_system_admin() && isset($_POST['delete_id'])) //only allow site admin to delete
{
	...
	sis_query_delete_cascade('si_acad_career', $_POST['delete_id'], $cascade);
}

echo $OUTPUT->header();
//content code starts here

sis_ui_page_title(get_string('academic_career','local_sis'));
$currenttab = 'academic_career'; //change this according to tab
include('tabs.php');
echo $OUTPUT->box_start('sis_tabbox');

$add_url = new moodle_url('/local/sis/setting/program/add_academic_career.php', array('action' => '1'));
echo '<div class="pull-right rc-attendance-teacher-print">' . html_writer::link($add_url, sis_ui_icon('plus-circle', '1', true) . ' ' . get_string('add_academic_career', 'local_sis'), array('title' => get_string('add_academic_career', 'local_sis'))) . '</div>';
...
$options = array(
	...
	'default_sort_field' => 'acad_career',
	...
	'default_search_field' => 'acad_career', //default field choose for search
	'view_page' => '',
	'edit_page' => 'add_academic_career.php',
...
$fields = array(
	'#' => array(), //# for numbering
	'acad_career' => array(
		'header'=>get_string('acad_career', 'local_sis'), //for custom header
		'align' => 'left',
		'size' => '15%',
...
//		'disable_search' => true,
	...
	'name_a' => array(
		'header'=>get_string('name') . ' (' . get_string('arabic', 'local_sis') . ')', //for custom header
		'align' => 'left',
		'size' => '20%',
	),
	'eff_status' => array(
...
//output the table
echo sis_ui_dump_table('si_acad_career', $options, $fields, 'local_sis');
