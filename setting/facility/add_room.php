...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/facility/add_room.php', $urlparams);
...
$PAGE->navbar->add(get_string('room', 'local_sis'), new moodle_url('index.php'));
$PAGE->navbar->add(get_string($bc, 'local_sis'), new moodle_url('add_room.php' . $qs));

$return_params = sis_get_session('si_room_return_params', $_GET);
if($return_params == '')
	$return_params = array();
...
{		
	...
	$duplicate = $DB->get_record('si_room', $duplicate_condition);
	if($duplicate) //found a record, possible duplicate
	{
		if($data->id != '') //updating
		{
			$original = $DB->get_record('si_room', array('id' => $data->id));
	...
	if(!$isDuplicate) //no duplicate, update it
	{
		...
		if($data->id == '') //create new
			$DB->insert_record('si_room', $data);	
		else
			$DB->update_record('si_room', $data);			
		redirect($return_url);
	}
}
...
if($isDuplicate)
	sis_ui_alert(get_string('duplicate_room', 'local_sis'), 'danger');
...
{
	$id = $_GET['id'];
	$toform = $DB->get_record('si_room', array('id' => $id));
	if($toform)
