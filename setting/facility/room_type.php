...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/facility/room_type.php', $urlparams);
...
sis_ui_page_title(get_string('room_type','local_sis'));
$option = 0;
...
{
	$post_data = $_POST;
	sis_set_session('room_type_search', $post_data);
}
else
{
	$post_data = sis_get_session('room_type_search');
...
$currenttab = 'room_type'; //change this according to tab
include('tabs.php');
...
$add_url = new moodle_url('/local/sis/setting/facility/add_room_type.php', array('action' => '1'));
echo '<div class="pull-right rc-attendance-teacher-print">' . html_writer::link($add_url, sis_ui_icon('plus-circle', '1', true) . ' Add Room Type', array('title' => 'Add Room Type')) . '</div>';

$form = sis_lookup_search_form($post_data);
sis_ui_box('', $form);

$where = sis_query_institute_where() . " and category = 'facility' && subcategory = 'room_type'"; //initialize a where clause with institute
if($post_data['search'] != '')
{
...
foreach($templates as $template)
{
	...
	$delete_url = "javascript:delete_record_room_type('$template->id')";
	$update_url = new moodle_url('/local/sis/setting/facility/add_room_type.php', array('id'=>$template->id));		
	
	$data[] = html_writer::link($delete_url, sis_ui_icon('trash', '1.5', true), array('title' => 'Delete Room Type')) . '&nbsp;' . 
			  html_writer::link($update_url, sis_ui_icon('pencil', '1.5', true), array('title' => 'Update Room Type'));
	$table->data[] = $data;
	unset($data);				
	$count++;
}
