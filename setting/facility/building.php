...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/facility/building.php', $urlparams);
...
$PAGE->navbar->add(get_string('building', 'local_sis'), new moodle_url('building.php'));

//Any form processing code
if(sis_is_system_admin() && isset($_POST['delete_id'])) //delete
{
	$DB->delete_records('si_building', array('id' => $_POST['delete_id']));
}

echo $OUTPUT->header();
//content code starts here
sis_ui_page_title(get_string('building','local_sis'));

$currenttab = 'building'; //change this according to tab
include('tabs.php');

echo $OUTPUT->box_start('sis_tabbox');

$add_url = new moodle_url('/local/sis/setting/facility/add_building.php', array('action' => '1'));
echo '<div class="pull-right rc-attendance-teacher-print">' . html_writer::link($add_url, sis_ui_icon('plus-circle', '1', true) . ' Add Building', array('title' => 'Add Building')) . '</div>';
...
$options = array(
	...
	'default_search_field' => 'building', //default field choose for search
	'view_page' => '',
	'edit_page' => 'add_building.php',
...
//output the table
echo sis_ui_dump_table('si_building', $options, $fields, 'local_sis');
