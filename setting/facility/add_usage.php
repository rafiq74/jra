...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/facility/add_usage.php', $urlparams);
...
*/
//we don't allow user to define usage as usage is a reserved word
$access_rules = array(
	'system' => ''
...
$PAGE->navbar->add(get_string($bc, 'local_sis'), new moodle_url('add_usage.php' . $qs));

//put before header so we can redirect
$mform = new usage_form();
...
{	
	$isDuplicate = si_lookup_duplicate($data->value, $data->lang, 'facility', 'usage', sis_get_institute(), $data->id);
	if(!$isDuplicate) //no duplicate, update it
	{
		if($data->id == '') //create new
			si_lookup_insert($data->value, $data->lang, 'facility', 'usage', sis_get_institute());
		else
			si_lookup_update($data->id, $data->value, $data->lang, 'facility', 'usage', sis_get_institute());
		redirect('usage.php');
	}
}
