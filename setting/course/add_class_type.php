...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/course/add_class_type.php', $urlparams);
...
$PAGE->navbar->add(get_string($bc, 'local_sis'), new moodle_url('add_class_type.php' . $qs));

$return_params = sis_get_session('si_lookup_return_params', $_GET);
if($return_params == '')
	$return_params = array();

$return_url = new moodle_url('class_type.php', $return_params);

//put before header so we can redirect
$mform = new class_type_form();
...
{	
	...
	$isDuplicate = si_lookup_duplicate($data->value, $data->lang, 'course', 'class_type', $institute, $data->id);
	if(!$isDuplicate) //no duplicate, update it
	{
		if($data->id == '') //create new
			si_lookup_insert($data->value, $data->lang, 'course', 'class_type', $institute);
		else
			si_lookup_update($data->id, $data->value, $data->lang, 'course', 'class_type', $institute);
		redirect($return_url);
	}
}
