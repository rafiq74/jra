...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/course/class_type.php', $urlparams);
...
$PAGE->navbar->add( get_string('class_type','local_sis'), new moodle_url('class_type.php'));
...
sis_ui_page_title(get_string('class_type','local_sis'));
$currenttab = 'class_type'; //change this according to tab
include('tabs.php');
echo $OUTPUT->box_start('sis_tabbox');

$add_url = new moodle_url('/local/sis/setting/course/add_class_type.php', array('action' => '1'));
echo '<div class="pull-right rc-attendance-teacher-print">' . html_writer::link($add_url, sis_ui_icon('plus-circle', '1', true) . ' ' . get_string('add_class_type', 'local_sis'), array('title' => get_string('add_class_type', 'local_sis'))) . '</div>';

$condition = array('institute' => sis_get_institute(),
				   'category' => 'course',
				   'subcategory' => 'class_type',
			);
...
$fields = array(
	'#' => array(), //# for numbering
	'value' => array(
		'header'=>get_string('class_type', 'local_sis'), //for custom header
		'align' => 'left',
		'size' => '70%',
		'lang' => true,
//		'format' => 'date',
