...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/organization/view_organization.php', $urlparams);
...
$PAGE->navbar->add(sis_get_string(['view', 'organization']), new moodle_url('view_organization.php'));

echo $OUTPUT->header();//content code starts here
//content code starts here
sis_ui_page_title(sis_get_string(['view', 'organization']));
...
$return_params = sis_get_session('si_organization_return_params');
if($return_params == '')
	$return_params = array();
...
$organization = $DB->get_record('si_organization', array('id' => $id));
if($organization)
{
	$detail_data = array();
	//one row of data
	...
	$detail_obj->column = 2;
	$detail_obj->left_content = '<strong>' . get_string('organization', 'local_sis') . '</strong>';
	$detail_obj->right_content = $organization->organization;
	...
	$detail_obj->right_content = $organization->organization_name;
	$detail_data[] = $detail_obj;
	//end of data row
	
	//display the positions
	$content = sis_setting_organization_post($organization, 'acad_org');
	//one row of data
	$detail_obj = new stdClass();
	$detail_obj->column = 2;
