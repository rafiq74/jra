...
$PAGE->navbar->add(get_string($bc, 'local_sis'), new moodle_url('add_organization.php' . $qs));
...
{		
	...
	$isDuplicate = false;
	$duplicate = $DB->get_record('si_organization', array('organization' => $data->organization, 'campus' => $data->campus, 'institute' => $data->institute));
	if($duplicate) //found a record, possible duplicate
	{
		if($data->id != '') //updating
		{
			$original = $DB->get_record('si_organization', array('id' => $data->id));
...
if($isDuplicate)
	sis_ui_alert(get_string('duplicate_organization', 'local_sis'), 'danger');
