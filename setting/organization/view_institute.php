...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/organization/view_institute.php', $urlparams);
...
$PAGE->navbar->add(sis_get_string(['view', 'institute']), new moodle_url('view_institute.php'));

echo $OUTPUT->header();//content code starts here
//content code starts here
sis_ui_page_title(sis_get_string(['view', 'institute']));
...
$return_params = sis_get_session('si_institute_return_params');
if($return_params == '')
	$return_params = array();
$return_url = new moodle_url('institute.php', $return_params);
...
$institute = $DB->get_record('si_institute', array('id' => $id));
if($institute)
{
	$detail_data = array();
	//one row of data
	...
	$detail_obj->column = 2;
	$detail_obj->left_content = '<strong>' . get_string('institute', 'local_sis') . '</strong>';
	$detail_obj->right_content = $institute->institute;
	...
	$detail_obj->right_content = $institute->institute_name;
	$detail_data[] = $detail_obj;
	//end of data row
	
	//display the positions
	$content = sis_setting_organization_post($institute, 'institute');
	//one row of data
	$detail_obj = new stdClass();
	$detail_obj->column = 2;
