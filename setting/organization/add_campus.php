...
$urlparams = $_GET;
$PAGE->set_url('/local/sis/setting/organization/add_campus.php', $urlparams);
...
$PAGE->navbar->add(get_string($bc, 'local_sis'), new moodle_url('add_campus.php' . $qs));

//put before header so we can redirect
$mform = new campus_form();
...
{	
	...
	$isDuplicate = false;
	$duplicate = $DB->get_record('si_campus', array('campus' => $data->campus, 'institute' => $data->institute));
	if($duplicate) //found a record, possible duplicate
	{
		if($data->id != '') //updating
		{
			$original = $DB->get_record('si_campus', array('id' => $data->id));
	...
	if(!$isDuplicate) //no duplicate, update it
	{
		...
		if($data->id == '') //create new
			$DB->insert_record('si_campus', $data);	
		else
			$DB->update_record('si_campus', $data);			
		redirect('campus.php');
	}
}
...
echo $OUTPUT->header();
if($isDuplicate)
	sis_ui_alert(get_string('duplicate_campus_code', 'local_sis'), 'danger');
//content code starts here
sis_ui_page_title(get_string($bc, 'local_sis'));

if($id)
{
	$toform = $DB->get_record('si_campus', array('id' => $id));
	if($toform)
