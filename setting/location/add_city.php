...
$PAGE->navbar->add(get_string($bc, 'local_sis'), new moodle_url('add_city.php' . $qs));
...
$return_params = sis_get_session('si_city_return_params', $_GET);
if($return_params == '')
	$return_params = array();
...
{	
	...
	$isDuplicate = false;
	$duplicate = $DB->get_record('si_city', array('city' => $data->city, 'state_id' => $data->state_id));
	if($duplicate) //found a record, possible duplicate
	{
		if($data->id != '') //updating
		{
			$original = $DB->get_record('si_city', array('id' => $data->id));
	...
	if(!$isDuplicate) //no duplicate, update it
	{
		...
		if($data->id == '') //create new
			$DB->insert_record('si_city', $data);	
		else
			$DB->update_record('si_city', $data);			
		redirect($return_url);
	}
}
...
if($isDuplicate)
	sis_ui_alert(get_string('duplicate_city', 'local_sis'), 'danger');
...
{
	$id = $_GET['id'];
	$toform = $DB->get_record('si_city', array('id' => $id));
	if($toform)
