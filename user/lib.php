...
function sis_user_update_password($m_user, $password)
{
	global $DB;
	$user = $DB->get_record('si_user', array('id' => $m_user->idnumber));
	if($user)
	{
		$user->password = sis_user_password_hash($password);
...
function sis_user_password_hash($pswd)
{
	return md5($pswd);
}

//reset the si_user password to the default system password
function sis_user_reset_password($id)
{
	global $DB;
	$var_name = 'system_default_password';
	$reset_option = sis_get_config($var_name);
	$pswd = '';
	if($reset_option == 'civil_id')
	{
		$rec = $DB->get_record('si_user', array('id' => $id));
		if($rec)
		{
			$pswd = $rec->national_id;				
		}
	}
	else if($reset_option == 'custom')
	{
		$var_name = 'system_default_password_custom';
		$pswd = sis_get_config($var_name);
	}
	if($pswd != '')
	{
		...
		if($user)
		{
			...
			if($m_user)
			{
				if(sis_allow_password_change($m_user))
				{
					$var_name = 'system_force_password_change';
					$force_change = sis_get_config($var_name);
				}
				else
					$force_change = 'N';
			}
			...
			$user->password = sis_user_password_hash($pswd);
...
function sis_user_send_password_change_info($user, $si_user) {
    global $CFG, $DB;
	...
	$si_user->token = $token;
	$si_user->token_date = $token_end_time;
	$DB->update_record('si_user', $si_user);
	...
	$url = new moodle_url($CFG->wwwroot . '/local/sis/user/reset_password.php', array('token' => $token));
