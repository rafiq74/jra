...
$urlparams = $_GET;
$PAGE->set_url('/local/jra/admin/semester/add_semester.php', $urlparams);
...
$PAGE->navbar->add(jra_get_string($bc), new moodle_url('add_semester.php', $urlparams));


$return_params = jra_get_session('si_semester_return_params');
if($return_params == '')
	$return_params = array();
$return_url = new moodle_url('index.php', $return_params);

//put before header so we can redirect
$mform = new semester_form();
...
if($isDuplicate)
	jra_ui_alert(get_string('duplicate_semester', 'local_jra'), 'danger');
...
{
	$id = $_GET['id'];
	$toform = $DB->get_record('si_semester', array('id' => $id));
	if($toform)
